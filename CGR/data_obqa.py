...
class AMRDataLoader(object):
    def __init__(self, args, fact_data, model_tokenizer, retriever_tokenizer, evaluate=False, test=False):
        ...
        if len(fact_data) == 6:
            core_f, core_vec, core_dict, comm_f, comm_vec, comm_dict = fact_data
            self.fact_dict = {"core":core_f, "comm":comm_f}
            self.amr_dict = {"core":core_dict, "comm":comm_dict}
            self.vec_dict = {"core":core_vec, "comm":comm_vec}
        elif len(fact_data) == 3:
            comm_f, comm_vec, comm_dict = fact_data
    ...
    def retrieve_fact(self, retriever):
        if self.datasetID == 'train':
            ...
            index_core = init_index(self.vec_dict['core'], vec_size)
            index_comm = init_index(self.vec_dict['comm'], vec_size)
            qa_post = {}
            for x in self.qa_f:

                qa_post[x] = self.retrieve_fact_faiss(retriever, self.qa_f[x], index_all=(index_core, index_comm))
                acc_c, acc_f3 = statis(qa_post[x])
                logger.info(x + " retriever: acc: {}. f3: {}.".format(acc_c, acc_f3))
            ...
            if os.path.exists(temp_path):
                ...
                self.qa_post = self.retrieve_fact_faiss(retriever, self.qa_f[self.datasetID], index_all=(index_core, index_comm))
                acc_c, acc_f3 = statis(self.qa_post)
                logger.info(self.datasetID + " retriever: acc: {}. f3: {}.".format(acc_c, acc_f3))

    def retrieve_fact_faiss(self, retriever, qa_f, index_all):
        Ilast = None
        Icomm = []
        t = 0
        index_core, index_comm = index_all
        hypos = [choice['hypo'] for data_one in qa_f for choice in data_one['question']['choices']]
        num_example = len(hypos)
        while t < self.T:
            if t == 0:
                query = prepare_query(hypos, self.fact_dict['core'], Ilast=Ilast)
                query_vec = get_vec(query, self.retriever_tokenizer, retriever, batch_size=128)
                Dcore, Icore = index_core.search(query_vec, self.kfact)
        ...
        for i_e, example in enumerate(qa_f):
            ...
            for i_c, choice in enumerate(choices):
                core_index_one = Icore[current_step + i_c]
                core_text_one = [self.fact_dict['core'][x] for x in core_index_one]
                core_amr_one = [self.amr_dict['core'][x] for x in core_text_one]
                for i_x, x in enumerate(core_text_one):
                    if not x.endswith("."):
                        core_text_one[i_x] = x + '.'
                ...
                para = core_text_one + comm_text_one
                para_amr = core_amr_one + comm_amr_one

                assert len(para) == len(para_amr)

                index_one = [('core', str(x)) for x in core_index_one] + [('comm', str(x)) for x in comm_index_one]
        ...
        for batch in batches:
            ...
            if self.datasetID == "train":
                ...
                for data_one in batch:
                    ...
                    if data_one["fact1"] is not None:
                        gold1 = self.vec_dict['core'][self.fact_dict['core'].index(data_one['fact1'])]
                        batch_fact_gold1.append(gold1)
                    ...
                    if len(choice['para_iter']) != 0:
                        ...
                        chain_iter_query = [" [SEP] ".join([hypo_one] + chain_iter[:i_i]) for i_i, index_one in enumerate(chain_iter)] \
                                           + [" [SEP] ".join([hypo_one] + chain_iter_re[:i_i]) for i_i, index_one in enumerate(chain_iter_re)]
                        chain_iter_fact = [self.vec_dict[x[0]][int(x[1])] for x in sampled_chain] +\
                                          [self.vec_dict[x[0]][int(x[1])] for x in reversed(sampled_chain)]
                ...
                batch_fact = batch_fact_gold1 + batch_chain_global + batch_chain_iter_fact
                if self.fp_16:
